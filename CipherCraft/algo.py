...
def save_key_file(key_path, private_key, public_key, encrypted_aes_key, password=None):
    ...
    with open(key_path, "w") as key_file:
        key_file.write(obfuscate(private_key.private_bytes(
            encoding=serialization.Encoding.PEM,
            format=serialization.PrivateFormat.PKCS8,
            encryption_algorithm=encryption_algorithm
        )) + "\n")
        key_file.write(obfuscate(public_key.public_bytes(
            encoding=serialization.Encoding.PEM,
...
def initialize_key_file(key_path, password):
    ...
    if not os.path.exists(key_path):
        ...
        save_key_file(key_path, private_key, public_key, rsa_encrypt(aes_key, public_key), password)
...
def encrypt_data(key_path, password, data, print_data=True):
    try:
        ...
        encrypted_data = aes_encrypt(base64.b64encode(data.encode() if isinstance(data, str) else data), hashlib.sha256(aes_key + hash_key_file(key_path)).digest())
...
def decrypt_data(key_path, password, encrypted_data, print_data=True):
    try:
        private_key, public_key, encrypted_aes_key = load_key_file(key_path, password)
        aes_key = rsa_decrypt(encrypted_aes_key, private_key)
        decrypted_data = base64.b64decode(aes_decrypt(base64.urlsafe_b64decode(encrypted_data), hashlib.sha256(aes_key + hash_key_file(key_path)).digest()))
        if print_data:
...
def encrypt_file(key_path, password, file_path):
    try:
        with open(sanitize_path(file_path), "rb") as data_file:
            encrypted_data = encrypt_data(key_path, password, data_file.read(), print_data=False)
            if encrypted_data:
...
def decrypt_file(key_path, password, file_path):
    try:
        ...
        with open(sanitize_path(file_path), "rb") as encrypted_file:
            decrypted_data = decrypt_data(key_path, password, base64.urlsafe_b64encode(encrypted_file.read()).decode(), print_data=False)
            if decrypted_data:
